import { createCustomCombinedEvaluatorsFor, createDataStructure, Maxim } from "@maximai/maxim-js";
...
const customCombinedProcessingEvaluator = createCustomCombinedEvaluatorsFor("minimum-sentence-evaluator", "specialCharactersCounter").build<
...
const result = await maxim
  .createTestRun(`custom combined processing evaluator test on ${Date.now()}`, workspaceId)
...
async function mockLLMEvaluationCall(output: string, context?: string | string[]) {
  ...
  const specialCharactersCountInOutput = output.match(/[!@#$%^&*(),.?":{}|<>]/g)?.length ?? 0;
